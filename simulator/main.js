...
function validateVars(appVars, info) {
  return info.presentation_properties.every(function(prop) {
    if (!Object.keys(appVars).includes(prop.name)) {
      console.log(prop.name + ' not in vars (' + JSON.stringify(appVars) + ')');
      return false;
    }
...
function main() {
  ...
  fileContents('.', program.varfile, JSON.parse) || {};
